...
class WanVideoVAE(nn.Module):
    ...
    def encode(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16),
        ...
        # keep_on_device=True: skip CPU offload, run entirely on GPU (fast, for training)
        # keep_on_device=False (default): offload to CPU first (slow but saves VRAM, for inference)
        if torch.is_grad_enabled() or keep_on_device:
            videos_list = [video for video in videos] if isinstance(videos, (list, tuple)) else [videos]
        else:
            videos_list = [video.to("cpu") for video in videos] if isinstance(videos, (list, tuple)) else [videos.to("cpu")]
        
        hidden_states = []
        for video in videos_list:
            video = video.unsqueeze(0) if video.dim() == 4 else video
            if tiled:
