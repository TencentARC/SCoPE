...
try:
    ...
    # ImportError can occur if flash-attn is compiled with different PyTorch version
    print(f"Warning: flash_attn not available ({type(e).__name__}), will use fallback attention")
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False):
    # FlashAttention only supports fp16 and bf16, not fp32
    # If input is fp32 and we're using FlashAttention, convert to fp16/bf16
    original_dtype = q.dtype
    needs_conversion = original_dtype == torch.float32 and (FLASH_ATTN_3_AVAILABLE or FLASH_ATTN_2_AVAILABLE) and not compatibility_mode
    
    if needs_conversion:
        # Prefer bf16 if available, otherwise use fp16
        if q.device.type == 'cuda' and torch.cuda.is_bf16_supported():
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, control_camera_dit_input=None):
        ...
        if control_camera_dit_input is not None:
            if hasattr(self, "cam_encoder") and hasattr(self, "projector"):
                ...
                input_x = input_x + y
                residual = self.projector(self.self_attn(input_x, freqs))
            elif hasattr(self, "cam_self_attn"):
                if self.cam_self_attn.adaptation_method == "before":
                    input_x = input_x + self.cam_self_attn(input_x, control_camera_dit_input)
                residual = self.self_attn(input_x, freqs)
                if self.cam_self_attn.adaptation_method == "parallel":
                    residual = residual + self.cam_self_attn(input_x, control_camera_dit_input)
            else:
        ...
        if control_camera_dit_input is not None \
            ...
            x = x + self.cam_self_attn(x, control_camera_dit_input)
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
                ...
                control_camera_dit_input: Optional[torch.Tensor] = None,
