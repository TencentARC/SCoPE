...
class ModelManager:
    ...
    def load_model(self, file_path, model_names=None, device=None, torch_dtype=None):
        ...
        if file_path is None:
            print(f"Warning: file_path is None, skipping...")
            return
        if isinstance(file_path, list) and len(file_path) == 0:
            print(f"Warning: file_path is empty list, skipping...")
            return
        ...
        for model_detector in self.model_detector:
            # If file_path is a list, pass the first element or a string representation for matching
            if isinstance(file_path, list):
                if len(file_path) == 0:
                    continue  # Skip empty list
                match_path = file_path[0]
            else:
                match_path = file_path
            if model_detector.match(match_path, state_dict):
